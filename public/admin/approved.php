<?php
$service = SQLFunction::getAllApproveServices();
?>
    ...
    function renderTable(requestsToRender) {
        ...
        requestsToRender.forEach(request => {
            ...
            let actionButtons = `
                ...
                <button class="btn btn-success action-btn complete-btn" data-id="${request.id}">COMPLETE</button>
            `;
        ...
        document.querySelectorAll('.complete-btn').forEach(button => {
            button.addEventListener('click', function () {
                const requestId = this.getAttribute('data-id');
                updateRequestToComplete(requestId);
            });
        });
    }

    function updateRequestToComplete(requestId) {
        const formData = new FormData();
        formData.append('id', requestId);

        const xhr = new XMLHttpRequest();
        xhr.open('POST', 'axios/complete.php', true);
        xhr.onload = function () {
            if (xhr.status === 200) {
